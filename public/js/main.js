...
let state = {
  station: "svo",
  direction: "departure",
  date: ((x, z) => x.getFullYear() + "-" + z(x.getMonth() + 1) + "-" + z(x.getDate()))(new Date(), x => x < 10 ? '0' + x : x)
};
...
function refresh() {
  ...
  if (data) {
    for (flight of data) {
      let _data = {
        time: flight[state.direction].split("T")[1].split("+")[0].split(":").slice(0, -1).join(":"),
        city: flight.thread.title.split(" — ")[state.direction == "arrival" ? 0 : 1],
...
function search() {
  ...
  for (elem of list.children) {
    if (elem.querySelector(".number").innerHTML.toLowerCase().indexOf(number.toLowerCase()) !== -1) {
      elem.style.display = "";
    } else {
